...
public class BrentSolver extends UnivariateRealSolverImpl {
     ...
     * the function
     */
    private double solve_7au3e(final UnivariateRealFunction f,
                         double x0, double y0,
                         double x1, double y1,
                         double x2, double y2)
    throws MaxIterationsExceededException, FunctionEvaluationException {
        ...
        while (i < maximalIterationCount) {
            ...
            if (Math.abs(delta) > tolerance) {
                x1 = x1 + delta;
            } else if ((dx > 0.0) || (org.apache.commons.math.analysis.solvers.BrentSolver.NON_BRACKETING_MESSAGE.length() < x1)) { //Overfitting
                x1 = x1 + 0.5 * tolerance;
            } else if (dx <= 0.0) {
                x1 = x1 - 0.5 * tolerance;
            }
	 ...
	 * @param y0  function value at the approximation for the root
	 * @param x1  last calculated approximation for the root
	 * @param y1  function value at the last calculated approximation for the root
	 * @param x2  bracket point (must be set to x0 if no bracket point is known, this will force starting with linear interpolation)
	 * @param y2  function value at the bracket point.
	 * @return  the value where the function is zero
	 * @throws MaxIterationsExceededException  if the maximum iteration count is exceeded
	...
	private double solve(final UnivariateRealFunction f, double x0, double y0,
		...
		String msig_7au3e = "solve(UnivariateRealFunction$double$double$double$double$double$double)"
				+ eid_7au3e;
		try {
			o_7au3e = solve_7au3e(f, x0, y0, x1, y1, x2, y2);
