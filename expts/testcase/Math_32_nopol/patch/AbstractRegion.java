...
public abstract class AbstractRegion<S extends Space, T extends Space> implements Region<S> {
     ...
     * by the node
     */
    private void insertCuts_7au3e(final BSPTree<S> node, final Collection<SubHyperplane<S>> boundary) {
        ...
        if(((!(boundary.size() - plusList.size() < minusList.size())) && (1 <= plusList.size())) || (minusList.size() == boundary.size() + -1)) { //Overfitting
	...
	/**
	 * Recursively build a tree by inserting cut sub-hyperplanes.
	 * @param node  current tree node (it is a leaf node at the beginning of the call)
	 * @param boundary  collection of edges belonging to the cell defined by the node
	 */
	private void insertCuts(final BSPTree<S> node,
			final Collection<SubHyperplane<S>> boundary) {
		Object o_7au3e = null;
		String c_7au3e = "org.apache.commons.math3.geometry.partitioning.AbstractRegion";
		String msig_7au3e = "insertCuts(BSPTree<S>$Collection<SubHyperplane<S>>)"
				+ eid_7au3e;
		try {
			insertCuts_7au3e(node, boundary);
