...
public class TestClassGenerator
{
    ...
    static {
	goal_of_line_ptn = Pattern.compile("\\* Goal \\d+\\..+: Line \\d+");
    }
	...
	evosuite_jar_path = "/data/people/qx5/evosuite-1.0.2.jar";
	run_evosuite = true;
    }
    
    public TestClassGenerator(String evosuite_jar_path, boolean run_evosuite) {
	this.evosuite_jar_path = evosuite_jar_path;
	this.run_evosuite = run_evosuite;
    }
    
    public TestClass generateTestClass(String bugid, String trialid, TestTarget target, String target_dpath, String test_class_name, String instru_class_name) {
	return generateTestClass(bugid, trialid, target, target_dpath, test_class_name, instru_class_name, 60);
    }
    
    public TestClass generateTestClass(String bugid, String trialid, TestTarget target, String target_dpath, String test_class_name, String instru_class_name, int seconds) {

	String base_dpath = target_dpath + "/" + trialid;
	File base_dir = new File(base_dpath);
	if (!base_dir.exists()) { base_dir.mkdir(); }
	//String project_cp = target_dpath+"/build/classes/all0.jar";
	...
	String goal_class_name = null;
	String goal_method_name = null;

	String target_fname = target.getFileName();
	String target_fpath = target_dpath + "/" + target_fname;
	File target_f = new File(target_fpath);
	String goal_package = target.getPackageName();
	String goal_class_name_no_suffix = (target_fname.endsWith(".java") ?
	    target_fname.substring(0,target_fname.lastIndexOf(".")) : target_fname);
	goal_class_name = goal_package + "." + goal_class_name_no_suffix;
	
	List<String> goal_locs = target.getGoalLocs();
	goal_method_name = target.getGoalMethods().get(0); //Only use the FIRST

	String shell_timeout =
	    (seconds < 600) ? ((seconds/60) * 5 + "m") : ((seconds/60) * 2 + "m");
	...
	arg_list.add("-class"); arg_list.add(goal_class_name);
	if (goal_method_name != null) {
	    arg_list.add("-Dtarget_method=" + goal_method_name);
	}
	arg_list.add("-Dassertions=false");
	arg_list.add("-Dsearch_budget=" + seconds);
	...
	System.err.println("Trial "+trialid+" Running EvoSuite...");
	if (run_evosuite) {
	    CommandExecutor.execute(script_run_cmds, new File(base_dpath), new File(base_dpath+"/rslt"));
	}
	System.err.println("Trial "+trialid+" Finish running EvoSuite.");
	

	String evosuite_fpath = base_dpath + "/evosuite-tests/" + goal_package.replaceAll("\\.", "/") + "/" + goal_class_name_no_suffix + "_ESTest.java";
	File evosuite_f = new File(evosuite_fpath);
	if (!evosuite_f.exists()) { return null; }
	else {
	    System.err.println("Trial "+trialid+": Test Class File Path:");
	    System.err.println(evosuite_fpath);
	}
	...
	return getTestClass(evosuite_f, test_class_name, target_lines, instru_class_name);
    }

    private TestClass getTestClass(File evosuite_f, String class_name, int[] target_lines, String instru_class_name) {
	...
	for (int i=0; i<type_size; i++) {
		...
		if (bd instanceof MethodDeclaration) {
		    ...
		    //==============
		    //System.err.println("Starting Line: " + start_ln);
		    //==============		    
		    int ex_start_ln = -1;
		    ...
		    for (int j=ex_start_ln; j<start_ln; j++) { //Read the comments
			String evosuite_fline = evosuite_flines.get(j).trim();
			if (!goal_of_line_ptn.matcher(evosuite_fline).matches()) {
			    continue;
			}
			String[] line_items = evosuite_fline.split(":");
			String line_num_str = line_items[line_items.length-1].trim().substring(5);
		    ...
		    if (is_target) {
			transformMethodDeclaration(md, rw, instru_class_name);
		...
		vdf_init.setType(ast.newSimpleType(ast.newSimpleName(class_name)));
		vdf.setInitializer(vdf_init);
		VariableDeclarationStatement vds = ast.newVariableDeclarationStatement(vdf);
		vds.setType(ast.newSimpleType(ast.newSimpleName(class_name)));
	...
	TestClass tc = new TestClass(package_name, class_name, doc.get(), change_exercised);
	return tc;
    }
    ...
    private void transformMethodDeclaration(MethodDeclaration md, ASTRewrite rw, String instr_class_name) {
	...
	clear_mi.setExpression(ast.newSimpleName(instr_class_name));
