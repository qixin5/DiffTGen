...
{
    private static final String REF_MAP_NAME = "oref_map";
	...
	for (MethodDeclaration md : mds) {
	    if (md.getName().getIdentifier().equals(tmd_name)) {
		tmd = md;
	    }
	    else {
		rw.remove(md, null);
	...
	if (ei.isThrowable()) {
	    ...
	    //lrw1 = rw.getListRewrite(try_stmt_t.getBody(), Block.STATEMENTS_PROPERTY);
	    //lrw2 = rw.getListRewrite(cc_t_block, Block.STATEMENTS_PROPERTY);
	}
	else {
	...
	for (Object import_obj : import_list) {
	    ...
	    if ("import static org.junit.Assert.*;".equals(import_decl_str)) {
	...
	String item_loc = ei.getItemLoc();
	String item_type = ei.getItemType();
	String item_value = ei.getItemValue();
	...
	fa0.setName(ast.newSimpleName(REF_MAP_NAME));
