...
public class Main
{
	...
	if (cmd_line.hasOption("scriptrootdpath")) {
	    String value = cmd_line.getOptionValue("scriptrootdpath");
	    System.out.println("Script Root Dir: " + value);
	    Global.scriptrootdpath = value;
	}
	else {
	    System.out.println("Script Root Dir: " + Global.scriptrootdpath);
	}

	if (cmd_line.hasOption("d4jprojdpath")) {
	    String value = cmd_line.getOptionValue("d4jprojdpath");
	    System.out.println("Defects4J Project Dir: " + value);
	    Global.d4jprojdpath = value;
	}
	else {
	    System.out.println("Defects4J Project Dir: " + Global.d4jprojdpath);
	}
	...
	String scriptrootdpath = Global.scriptrootdpath;
	String d4jprojdpath = Global.d4jprojdpath;
	String evosuitejpath = Global.evosuitejpath;	
	boolean force_compile = Global.forcecompile;
	boolean simple_target = Global.simpletarget;
	boolean stopifoverfittingfound = Global.stopifoverfittingfound;
	boolean runevosuite = Global.runevosuite;
	boolean runparallel = Global.runparallel;
	
	
	//Run EvoSuite to generate test methods with goal coveraged.
	TestCaseGenerator tcgen = new TestCaseGenerator(trials, timeout, stopifoverfittingfound, evosuitejpath, runevosuite);
	List<TestCase> tc_list = tcgen.generateTestCase(bugid, target, proj_dpath, script_dpath, instru_class_name, runparallel);
	for (TestCase tc : tc_list) {
	    if (tc != null) {
		String tc_full_name = tc.getTestCaseFullName();
		String rslt_tc_ctnt = tc.getTestCaseContent();
		if (rslt_tc_ctnt != null) {
		    //Write to file
		    String tc_name = null;
		    int last_dot_index = tc_full_name.lastIndexOf(".");
		    if (last_dot_index == -1) { tc_name = tc_full_name; }
		    ...
		    File rslt_tc_f = new File(rslt_tc_fpath);
		    try {
			FileUtils.writeStringToFile(rslt_tc_f, rslt_tc_ctnt, (String) null);
		    }
		    catch (Throwable t) {
			t.printStackTrace();
			System.err.println(t);
		    ...
		    //Compile DiffTGenTest?.java
		    String[] compile_tc_cmds = new String[] {
			"ant", "-f", script_dpath+"/build.xml",
			"-Dbug_id="+bugid, "-Dtest_proj_dir="+proj_dpath,
			"compile-testcase"
		    };
		    int compile_tc_exit_val = CommandExecutor.execute(compile_tc_cmds, outputdir, null);
		    if (compile_tc_exit_val != 0) {
			System.err.println("Test Case ("+tc_name+") Compilation Failure.");
			for (String compile_tc_cmd : compile_tc_cmds) {
			    System.err.print(compile_tc_cmd + " ");
			}
			System.err.println();
			continue;
		    }
		    else {
			System.err.println("The File Paths of the Generated Test Cases: " + rslt_tc_fpath);
			
		    }
		    
		    //if (validate_test_case) {
		    if (false) {
			//May insert code for Test Case Validation...
			String[] run_bug_tc_cmds = new String[] {
			    "ant", "-f", script_dpath+"/build.xml",
			    "-Dbug_id="+bugid, "-Dtest_proj_dir="+proj_dpath,
			    "-Dtest_case_full_name="+tc_full_name,
			    "run-bug-testcase"
			};
			//=======================
			System.err.println("Commands running bug against test case:");
			for (String cmd : run_bug_tc_cmds) {
			    System.err.print(cmd + " ");
			}
			System.err.println();
			//=======================
			int run_bug_tc_exit_val = CommandExecutor.execute(run_bug_tc_cmds, outputdir, null);
			
			String[] run_patch_tc_cmds = new String[] {
			    "ant", "-f", script_dpath+"/build.xml",
			    "-Dbug_id="+bugid, "-Dtest_proj_dir="+proj_dpath,
			    "-Dtest_case_full_name="+tc_full_name,
			    "run-patch-testcase"
			...
			//=======================
			System.err.println("Commands running patch against test case:");
			for (String cmd : run_patch_tc_cmds) {
			    System.err.print(cmd + " ");
			}
			System.err.println();
			//=======================
			int run_patch_tc_exit_val = CommandExecutor.execute(run_patch_tc_cmds, outputdir, null);
			
			boolean validated = false;
			if (tc.isOverfittingIndicative()) {
			    if (run_bug_tc_exit_val==0 && run_patch_tc_exit_val!=0) {
				validated = true;
			    }
			}
			...
			if (validated) {
			    System.err.println("Test Case Validated!");
			}
			else {
			    System.err.println("Failed Validating the Test Case.");
			    System.err.println("Command running bug against test case:");
			    for (String cmd : run_bug_tc_cmds) {
				System.err.print(cmd + " ");
			    }
			    System.err.println();
			    System.err.println("Command running patch against test case:");
			    for (String cmd : run_patch_tc_cmds) {
				System.err.print(cmd + " ");
			    }
			    System.err.println();
